...
| 改了代码想让线上服务生效 | 无（`git push origin main` 之后不用管） | GitHub Actions 跑测试，绿了推 `release` 分支，VPS 上的 systemd timer 1 分钟内自动拉取 + 重启 + 健康检查，不健康自动回滚 | `.github/workflows/ci.yml`、`scripts/deploy/` |
